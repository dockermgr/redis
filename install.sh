...
# Defaults variables
APPNAME="redis"
export APPDIR="$HOME/.local/share/srv/docker/redis"
export DATADIR="$HOME/.local/share/srv/docker/redis/rootfs"
export INSTDIR="$HOME/.local/share/CasjaysDev/dockermgr/redis"
...
HUB_IMAGE_URL="casjaysdevdocker/redis"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# image tag [docker pull HUB_IMAGE_URL:tag]
HUB_IMAGE_TAG="latest"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set the container name Default: [casjaysdevdocker/redis-$HUB_IMAGE_TAG]
CONTAINER_NAME=""
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set container hostname and domain - Default: redis
CONTAINER_HOSTNAME=""
...
if [ -z "$CONTAINER_NAME" ]; then
  CONTAINER_NAME="$(__container_name || echo "casjaysdevdocker/redis-$HUB_IMAGE_TAG")"
fi
...
if [ -n "$REDIS_USERNAME" ]; then
  CONTAINER_USER_NAME="$REDIS_USERNAME"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
  CONTAINER_USER_NAME="${REDIS_USERNAME:-${CONTAINER_USER_NAME:-$DEFAULT_USERNAME}}"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
